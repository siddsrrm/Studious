...
const gradeEntrySchema = new mongoose.Schema({
    ...
    type: { type: String, enum: ["assignment", "project", "exam"], required: true },
...
const gradeBookSchema = new mongoose.Schema({
    studyPlanID: { type: mongoose.Schema.Types.ObjectId, ref: "StudyPlan", required: true, unique: true },
...
gradeBookSchema.methods.calculateOverallGrade = function () {
    ...
    const hasWeights = this.entries.every(e => e.weight !== null && e.weight !== undefined);

    if (hasWeights) {
        const totalWeight = this.entries.reduce((sum, e) => sum + e.weight, 0);
        if (totalWeight === 0) return null;
        const weighted = this.entries.reduce((sum, e) => sum + e.score * e.weight, 0);
