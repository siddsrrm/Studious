...
describe("updateStudyPlan", () => {
  ...
  test("should return 404 if plan not found", async () => {
    ...
    await studyPlanController.updateStudyPlan(req, res);

    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith({ message: "Study plan not found" });
  });

  test("should return 403 if user does not own the plan", async () => {
  ...
  test("should update title, description, creditHours, and workload", async () => {
    const mockPlan = {
    ...
    expect(mockPlan.title).toBe("New Title");
    expect(mockPlan.description).toBe("New Desc");
    expect(mockPlan.creditHours).toBe(3);
    expect(mockPlan.workload).toBe(10);
    expect(mockPlan.save).toHaveBeenCalled();
    expect(res.json).toHaveBeenCalledWith(mockPlan);
  });

  test("should ignore invalid creditHours and workload", async () => {
    const mockPlan = {
      owner: { toString: () => "user123" },
      save: jest.fn().mockResolvedValue(true),
    };
    ...
    expect(mockPlan.creditHours).toBeUndefined();
    expect(mockPlan.workload).toBeUndefined();
  });
