...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        {studyPlans.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-700">No study plans yet</h2>
            <p className="text-gray-500 mt-1 mb-6 max-w-sm">
              Create your first study plan to start organising your notes, tasks, and practice questions.
            </p>
            <button
              onClick={() => setShowCreateForm(true)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {studyPlans.map((plan) => (
              <PlanCard key={plan.id} plan={plan} onSelect={handleSelectPlan} onDelete={handleDeletePlan} />
            ))}
          </div>
        )}
      </main>
      ...
      {showCreateForm && (
        <CreatePlanForm onCreatePlan={handleCreatePlan} onCancel={() => setShowCreateForm(false)} />
      )}
